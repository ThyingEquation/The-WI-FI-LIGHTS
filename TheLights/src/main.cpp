...
void (*modeFunctions[])(uint8_t) = { // порядок группы эффекта в этом массиве = коду группы эффекта
    ...
    drawRunningLine, drawAnimations, initEffectAllModes};
static const uint8_t GROUP_COUNT = sizeof(modeFunctions) / sizeof(modeFunctions[0]);

enum EffectsGroupMode // Главные группы эффектов. Порядяк эффектов аналогичен андроид приложению
{
  COLORFUL_EFFECTS,  // (0)  - Цветные эфффекты
...
constexpr auto ALL_MODES_EFFECT_COUNT = sizeof(mainModesData) / sizeof(mainModesData[0]);

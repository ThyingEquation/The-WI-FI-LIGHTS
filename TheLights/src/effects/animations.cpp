...
static const uint32_t *const explosionImage[] = {explosion_1,  explosion_2,  explosion_3,  explosion_4,  explosion_5,
                                                 ...
                                                 explosion_16, explosion_17, explosion_18};
...
const uint8_t mainMatrixScheme[] = {
        132, 131, 108, 107, 84, 83, 60, 59, 36, 35, 12, 11, 133, 130, 109, 106, 85, 82, 61, 58, 37, 34, 13, 10,
        134, 129, 110, 105, 86, 81, 62, 57, 38, 33, 14, 9,  135, 128, 111, 104, 87, 80, 63, 56, 39, 32, 15, 8,
        136, 127, 112, 103, 88, 79, 64, 55, 40, 31, 16, 7,  137, 126, 113, 102, 89, 78, 65, 54, 41, 30, 17, 6,
        138, 125, 114, 101, 90, 77, 66, 53, 42, 29, 18, 5,  139, 124, 115, 100, 91, 76, 67, 52, 43, 28, 19, 4,
        140, 123, 116, 99,  92, 75, 68, 51, 44, 27, 20, 3,  141, 122, 117, 98,  93, 74, 69, 50, 45, 26, 21, 2,
        142, 121, 118, 97,  94, 73, 70, 49, 46, 25, 22, 1,  143, 120, 119, 96,  95, 72, 71, 48, 47, 24, 23, 0};
...
void drawAnimations(const uint8_t subMode) {
...
void drawPicture(const uint32_t *p2) {
    ...
    for (uint16_t i = 0; i < MATRIX_LEDS; i++) {
        if (const uint8_t targetLed = mainMatrixScheme[i]; targetLed < MATRIX_LEDS) {
            uint32_t rawColor;
...
uint16_t xyToIndex(const uint16_t x, const uint16_t y) {
    if (x % 2 == 0) {
...
static void drawShifted(const uint32_t *image, const int8_t offsetY, const bool append) {
    if (image == nullptr)
        return;
    if (!append)
    ...
    for (int8_t row = 0; row < MATRIX_HEIGHT; row++) {
        const int8_t imgRow = static_cast<int8_t>(row + offsetY);
        ...
        for (int8_t col = 0; col < MATRIX_WIDTH; col++) {
            ...
            if (color > 0) {
                if (const uint8_t ledIndex = pgm_read_byte(&mainMatrixScheme[row * MATRIX_HEIGHT + col]);
...
static void processLetters(LetterAnimState &st, const uint32_t *const letters[], const uint8_t totalLetters) {
    ...
    switch (st.phase) {
        case 0:
            if (st.step <= MATRIX_HEIGHT) {
                drawShifted(letters[0], static_cast<int8_t>(MATRIX_HEIGHT - st.step), false);
                st.step++;
            } else {
                st.phase = 1;
                st.step = 0;
        ...
        case 1:
            if (st.currentLetter < totalLetters - 1) {
                if (st.step <= MATRIX_HEIGHT + 3) {
                    drawShifted(letters[st.currentLetter], static_cast<int8_t>(-st.step), false);
                    if (const int8_t nextOffset = static_cast<int8_t>(MATRIX_HEIGHT + 3 - st.step);
                        nextOffset <= MATRIX_HEIGHT) {
        ...
        case 2:
            if (st.step < 12) {
                drawShifted(letters[totalLetters - 1], static_cast<int8_t>(-st.step), false);
                st.step++;
            } else {
                st.phase = 3;
            }
...
void shiftLeft(const uint8_t a) {
    ...
    for (uint8_t row = 0; row < MATRIX_HEIGHT; row++) {
        for (uint8_t col = 0; col < MATRIX_WIDTH; col++) {
            const uint16_t index = (col + shift) % 12;
            uint32_t color;
            memcpy_P(&color, signalsImage[a] + (row * 12 + index), sizeof(uint32_t));
            leds[xyToIndex(col, row)] = CRGB(color);
        }
    }
    stripShow();
...
static void drawAnimation(const uint32_t *const *images, const uint32_t numImages, const uint32_t interval,

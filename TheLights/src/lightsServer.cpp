...
void initLightServer() {
    ...
    const IPAddress localIp(settings.localIpVal[0], settings.localIpVal[1], settings.localIpVal[2],
                            settings.localIpVal[3]);
    const IPAddress gateway(settings.gatewayVal[0], settings.gatewayVal[1], settings.gatewayVal[2],
                            settings.gatewayVal[3]);
    const IPAddress subnet(settings.subnetVal[0], settings.subnetVal[1], settings.subnetVal[2], settings.subnetVal[3]);
    delay(50);
...
void checkLightServer() {
    if (Udp.parsePacket()) {
        ...
        if (const int len = Udp.read(incomingPacket, 255); len > 0) {
            incomingPacket[len] = 0;
        }

        const std::string_view command = std::string_view(incomingPacket);

        if (constexpr std::array<std::string_view, 4> commands = {"connectionState?", "settings?", "canvas?",
                                                                  "command?"};
            command.rfind(commands[0]) == 0) {
...
static void handleConnectionState(const std::string_view command) {
    if (command == "check") {
        Udp.beginPacket(Udp.remoteIP(), Udp.remotePort());
        Udp.write("imHere");
        Udp.endPacket();
...
static void handleCanvas(std::string_view command) {
    ...
    if (colorStart == std::string_view::npos) {
        ...
        const std::string_view colorString = command.substr(colorStart + 6, colorEnd - (colorStart + 6));
    ...
    if (ledStart == std::string_view::npos) {
        ...
        const std::string_view ledString = command.substr(ledStart + 4);
